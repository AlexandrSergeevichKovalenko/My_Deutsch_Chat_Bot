...
async def send_morning_reminder(context: CallbackContext):
    message = (
        ...
        "🔹 В 07:00, 13:00 и 19:00 будут **промежуточные итоги** по каждому участнику.\n"
        "🔹 Итоговые результаты дня отправляются в 23:01.\n\n"
    )  
...
async def generate_sentences():
    ...
    **Требования:**
    - Ты — генератор русских предложений для изучения немецкого языка. Создавай осмысленные, грамматически правильные предложения с целевыми грамматическими структурами.
    - Используй **пассивный залог** и **Konjunktiv II** минимум в одном предложении. 
    - Используй Перечень тем: Глагол «lassen», Futur II, Субъективное значение модальных глаголов, Пассивный залог во всех временах и альтернативные конструкции, Существительные с управлением, Неопределённые местоимения, Прилагательные с управлением, Модальные частицы, Порядок слов в предложениях обстоятельствами времени причины образа действия, места, Все типы придаточных предложений
    - Используй Перечень тем: Konjunktiv 1 для выражения косвенной речи. Doppelte Konjunktionen: entweder...oder, zwar...aber, nicht nur...sondern auch, sowohl ...als auch, weder...noch, je...desto.
    - Используй Перечень тем: устойчивые глагольно-именные словосочетания, такие как: привести к успеху, Принять участие, Оказать помощь, Произвести впечатление, Осуществить контроль, Совершить ошибку, Иметь значение, Принять во внимание.
...
async def check_translation(original_text, user_translation):
    ...
    **Требования к проверке**:
    ...
    2. **Если оценка ниже 75 - обязательно объясни как должна правильно строится основная грамматическая конструкция данного предложения**.
    ...
    4. Для смыслового глагола укажи один наиболее часто встречаемый синоним в формате: Синоним:....

    **Формат ответа (без лишних символов, только текст!)**:
    Оценка: X/100
    Ошибки: ** объяснение необходимо только если оценка ниже 75, Если оценка выше либо равна 75 ошибки не разбираем.** 
    Верный перевод: ...
    Синоним: ... Основного смыслового глагола формате например erhalten/bekommen
    """
...
def main():
    ...
    # ✅ Итоги дня
    scheduler.add_job(lambda: run_async_job(send_daily_summary), "cron", hour=22, minute=2)

    # ✅ Итоги недели
    scheduler.add_job(lambda: run_async_job(send_weekly_summary), "cron", day_of_week="sun", hour=22, minute=22)
